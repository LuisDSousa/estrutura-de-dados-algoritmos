...
(21, 'Daniela Alves', '222.222.422-22', '1987-12-30', '11 9888-0006', 'daniela.alves@example.com', 'Av Med, 60');
...
JOIN
    pessoa p
    ON me.pessoa_id = p.id;

--ver os pacientes que n frequenta

SELECT 
    p.id AS pessoa_id,
    p.nome AS pessoa_nome,
    p.cpf AS pessoa_cpf,
    pa.pessoa_id AS pessoa_id,
    c.id
FROM
    paciente pa
JOIN pessoa p
...
LEFT JOIN
    consulta c
    ON pa.pessoa_id = c.paciente_id
WHERE
    c.id IS NULL;

--ver os medicos que n atenderam ninguem
SELECT 
    p.id AS pessoa_id,
    p.nome AS pessoa_nome,
    p.cpf AS pessoa_cpf,
    me.pessoa_id AS pessoa_id,
    c.id
FROM
    medico me
JOIN pessoa p
...
LEFT JOIN
    consulta c
    ON me.pessoa_id = c.medico_id
WHERE   
    c.id IS NULL;
    
--ver os medicos que n atenderam ninguem (versão simplificada)
SELECT 
    p.id AS pessoa_id,
    p.nome AS pessoa_nome,
    m.especialidade,
